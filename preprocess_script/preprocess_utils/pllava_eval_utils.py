...
class Conversation(EasyDict):
    ...
    def get_prompt(self):
        ...
        for i, (role, message) in enumerate(self.messages):
            if i+1 != len(self.messages): continue
...
conv_eval_ethucy0 = Conversation(
    system="",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
    mm_token='<image>\n',
    mm_style=MultiModalConvStyle.MM_INTERLEAF,
    pre_query_prompt="The input consists of a pedestrian scene with brown colored floor in the middle and snow covered on its side. People are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red dot.",
    post_query_prompt="\n",
    answer_prompt='\The person is'
)

conv_eval_ethucy = Conversation(
    system="",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
    mm_token='<image>\n',
    mm_style=MultiModalConvStyle.MM_INTERLEAF,
    pre_query_prompt="The input consists of a pedestrian scene where people are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red dot.",
    post_query_prompt="\n",
    answer_prompt='\The person is'
)

conv_eval_sit = Conversation(
    system="",
    roles=("USER: ", "ASSISTANT:"),
    messages=[],
    sep=[" ","</s>"],
    mm_token='<image>\n',
    mm_style=MultiModalConvStyle.MM_INTERLEAF,
    pre_query_prompt="The input consists of a pedestrian scene where people are standing still or walking, alone or in a group.",
    post_query_prompt="\n",
    answer_prompt='\The person is'
)
...
conv_templates = {
    ...
    "eval_ethucy0": conv_eval_ethucy0,
    "eval_ethucy": conv_eval_ethucy,
    "eval_sit": conv_eval_sit
}
