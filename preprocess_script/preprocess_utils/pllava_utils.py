...
from preprocess_script.preprocess_utils.pllava_eval_utils import Conversation
from preprocess_script.preprocess_utils.pllava import PllavaProcessor, PllavaForConditionalGeneration, PllavaConfig
...
def load_pllava(repo_id, num_frames, use_lora=False, weight_dir=None, lora_alpha=4, use_multi_gpus=False, pooling_shape=(16,12,12)):
    kwargs = {
...
def pllava_answer(conv: Conversation, model, processor, img_list, do_sample=True, max_new_tokens=200, num_beams=1, min_length=1, top_p=0.9,
               repetition_penalty=1.0, length_penalty=1, temperature=1.0, stop_criteria_keywords=None, print_res=False, prompt_override=None):
    # torch.cuda.empty_cache()
    # if prompt_override is None: 
    prompt = conv.get_prompt()
    # else: prompt = prompt_override
