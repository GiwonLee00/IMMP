...
frame_idx = "001000"
target_label_2d = "/mnt/minseok/analyze/dataset/jrdb/train_dataset/labels/labels_2d_stitched/bytes-cafe-2019-02-07_0.json"
target_label_3d = "/mnt/minseok/analyze/dataset/jrdb/train_dataset/labels/labels_3d/bytes-cafe-2019-02-07_0.json"
target_img = "/mnt/minseok/analyze/dataset/jrdb/train_dataset/images/image_stitched/bytes-cafe-2019-02-07_0/" + frame_idx + ".jpg"
...
def create_grid_lines_mesh(size=10, divisions=10, robot_pos=None):   
    ...
    for i in range(divisions + 1):
        ...
        if abs(robot_pos[1] - (y + start_y))<10.01:
            start = [robot_pos[0] - 10, y + start_y, -0.05]
            end = [robot_pos[0] + 10, y + start_y, -0.05]
    ...
    for i in range(divisions + 1):
        ...
        if abs(robot_pos[0] - (x + start_x))<10.01:
            start = [x + start_x, robot_pos[1] - 10, -0.05]
            end = [x + start_x, robot_pos[1] + 10, -0.05]
            prism = create_thick_line_prism(start, end, 0.05)
            start_index = len(vertices)
            vertices.extend(prism.vertices)
            # Adjust faces indices
...
def plot_3d_human(draw_frame_idx, person, ax, save_dir=None):
    ...
    offscreen_r = OffscreenRenderer(viewport_width=640*4, viewport_height=480*4)
    draw_person_idx = 0
    ...
    if save_dir != None:
        imsave(f"{save_dir}/frame_{draw_frame_idx}.jpeg", color, dpi=500)
