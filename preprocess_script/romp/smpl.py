...
class VertexJointSelector(nn.Module):
    ...
    def __init__(self, extra_joints_idxs, J_regressor_extra9, J_regressor_h36m17, dtype=torch.float32):
        ...
        self.extra_joints_idxs = self.extra_joints_idxs.cuda()
        self.J_regressor_extra9 = self.J_regressor_extra9.cuda()
        self.J_regressor_h36m17 = self.J_regressor_h36m17.cuda()
...
class SMPL(nn.Module):
    def __init__(self, model_path, model_type='smpl', dtype=torch.float32):
        ...
        self.lbs_weights = self.lbs_weights.cuda()
    ...
    def forward(self, betas=None, poses=None, root_align=False):
        ...
        if self.shapedirs.device != 'cuda': self.shapedirs = self.shapedirs.cuda()
        if self.v_template.device != 'cuda': self.v_template = self.v_template.cuda()
