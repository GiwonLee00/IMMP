...
class Caption3DPoseEngine:
    def __init__(self, conv_mode, num_frames):
        ...
        self.prompt = [
             'The input consists of a pedestrian scene with brown colored floor in the middle and snow covered on its side. People are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red bounding box. Describe, is the person standing still or walking? In the last frame, is there any obstacle in the facing direction of the person? In one sentence, give concise answer less than 20 words.',
             'This is a cctv footage, recorded from a bird eye view setting. Your job is to describe the behavior of the person marked with red bounding box. Describe, is the person standing still or walking? If the red bounding box is stationary, the person is standing still. In the last frame, is there any obstacle in the facing direction of the person? In one sentence, give concise answer less than 20 words.',
             'The input consists of a pedestrian scene. People are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red bounding box. Describe, is the person standing still or walking? In the last frame, is there any obstacle in the facing direction of the person? In one sentence, give concise answer less than 20 words.',
             'The input consists of a pedestrian scene. People are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red bounding box. Describe, is the person standing still or walking? In the last frame, is there any obstacle in the facing direction of the person? In one sentence, give concise answer less than 20 words.',
             'The input consists of a pedestrian scene. People are standing still or walking, alone or in a group. Your job is to describe the behavior of the person marked with red bounding box. Describe, is the person standing still or walking? In the last frame, is there any obstacle in the facing direction of the person? In one sentence, give concise answer less than 20 words.',
        ]
    ...
    def _crop_img_dot(self, img, pos, sceneIdx):
        ...
        # Draw dot
        # cv2.circle(img, pos, 4, (0, 0, 255), -1, cv2.LINE_AA)
        
        # Draw bounding box
        rec_pt1, rec_pt2 = self.get_bbox_pts(pos, sceneIdx)
        cv2.rectangle(img, rec_pt1, rec_pt2, (0, 0, 255), 2)
        ...
        if target_frame in self.traj_data.keys():
            for agent_id in self.traj_data[target_frame].keys():
                if agent_id not in self.agents.keys():
                    ...
                    cropped_img, full_img, pixel_pos = self._crop_img_dot(copy.deepcopy(img), temp.local_pos[target_frame], sceneIdx)
                    temp.frames[target_frame] = full_img
                    ...
                    cropped_img, full_img, pixel_pos = self._crop_img_dot(copy.deepcopy(img), self.agents[int(agent_id)].local_pos[target_frame], sceneIdx)
                    self.agents[int(agent_id)].frames[target_frame] = full_img
                    ...
                    for frame_key in self.agents[int(agent_id)].frames.keys():
                        if frame_key < target_frame - use_frame:
                            ...
                            del agents_copy[int(agent_id)].pixel_pos[frame_key]
    ...
    def caption_multiframe(self, frame_idx, sceneIdx, numFrames2use):
        for agent_id in self.agents.keys():
            ...
            agent_pos = np.zeros((0, 4))
            agent_vid = np.zeros((0, ORIGINAL_IMG_SIZE[0], ORIGINAL_IMG_SIZE[1], 3))
            ...
            for frame_net in range(numFrames2use,-1,-1):
                if frame_idx - frame_net in self.agents[agent_id].frames.keys():
                    agent_vid = np.concatenate((agent_vid, np.expand_dims(self.agents[agent_id].frames[frame_idx - frame_net], axis=0)), axis=0)
            ...
            # Crop out agent trajectory
            # [y_start, y_end, x_start, x_end]
            if agent_pos.shape[0] != 8:
                agent.caption = None
            else:
                y_start, y_end, x_start, x_end = agent_pos[:, 0].min(), agent_pos[:, 1].max(), agent_pos[:, 2].min(), agent_pos[:, 3].max()
                
                agent_vid = agent_vid[:, int(y_start):int(y_end), int(x_start):int(x_end)]
                
                # if self.caption_count != 2:
                ...
                print(f'#: {self.caption_count}, {llm_response}')
                for image_idx, image_ in enumerate(vid):
                    image_.save(f'/mnt/jaewoo4tb/textraj/temp/{self.caption_count}_iter{image_idx}_frame{frame_idx}_scene{sceneIdx}.png')
                # caption = caption[caption.find(next(filter(str.isalpha, caption))):]
                self.caption_count += 1
                # agent.caption = caption
            
            # img feature
            # crop_img_small = Image.fromarray(agent_vid[-1])
...
def load_video(video, num_segments=8, return_msg=False, num_frames=4, resolution=336):
    ...
    # print(f" **** The length of vr : {num_frames}")
    # print(f" **** The type of vr : {type(vr)}")
    # print(f" **** The type of vr[0] : {type(vr[0])}")
    # print(f" **** The type of vr[1] : {type(vr[1])}") 
