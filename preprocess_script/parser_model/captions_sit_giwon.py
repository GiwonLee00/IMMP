...
import copy
from preprocess_script.preprocess_utils.pllava_utils import *
from preprocess_script.preprocess_utils.pllava_eval_utils import conv_templates
import torchvision
    ...
    def preprocess_frame(self, target_frame, sceneIdx, use_frame):  # 7,0,8
        ...
        for annot_3d_ in annot3d_list:
            ...
            if 'Pedestrian' in annot_3d[0]:
                ...
                elif annot_3d[0] == 'Pedestrain_sitting':
                    agent_id = int(annot_3d[1].split(':')[1]+"999")     # NOTE: sitting case는 분리 (뒤에 999)
                if angle < 0: angle += 360
                assert angle > 0 and angle < 360
                annot3d[agent_id] = [annot_3d[5], annot_3d[6], annot_3d[8], angle]     # x, y, rot_z, angle around robot
        return annot3d, ego_pos, ego_yaw
        # breakpoint()
        '''
        ...
        for agent_id in annot3d.keys():
            ...
            if agent_id not in self.agents.keys():
                ...
                temp.type = 'pedestrian'        # sitting과 moving 모두 하나로 통합.
                ...
                self.agents[int(agent_id)] = temp       # 모든 값이 들어갈 수 있도록 해줌.
            else:
