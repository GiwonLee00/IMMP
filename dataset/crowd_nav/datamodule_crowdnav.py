...
from .dataset_crowdnav import ForecastDataset
import numpy as np
import random

class DataModule_CrowdNav():
    def __init__(self, args, cfg) -> None:
        self.args = args
        self.cfg = cfg
        
        horizon, history, plan_horizon = cfg["model"]["horizon"], cfg["model"]["history"], cfg["model"]["horizon"]
        self.train_dataset = ForecastDataset(data_root="data_preprocessed/CROWDNAV_processed", split="train")
        self.valid_dataset = ForecastDataset(data_root="data_preprocessed/CROWDNAV_processed", split="valid")
