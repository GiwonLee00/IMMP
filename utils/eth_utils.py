...
class preprocess(object):
    ...
    def __init__(self, data_root, seq_name, parser, split='train'):
        self.parser = parser
        ...
        self.init_frame = fr_start + parser.min_past_frames
        # self.num_fr = fr_end + 1 - fr_start
        self.num_fr = fr_end + 1 - fr_start - parser.min_future_frames
...
def get_ethucy_split(dataset):
     ...
     train, val = [], []
     for seq in seqs:
          if seq in test:        # Pretrain on all eth datas
               continue
          train.append(f'{seq}_train')
          val.append(f'{seq}_val')
     return train, val, test

def get_ethucy_split_all(dataset):
     seqs = [
          'biwi_eth',
          'biwi_hotel',
          'crowds_zara01',
